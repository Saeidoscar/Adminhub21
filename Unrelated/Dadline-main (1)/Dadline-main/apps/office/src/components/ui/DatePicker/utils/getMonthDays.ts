...
export function getMonthDays(month: Date): Date[][] {
  ...
  // تاریخ شروع ماه شمسی
  const startOfMonthJalali = moment(
    `${year}-${monthNumber}-01`,
    "jYYYY-jMM-jDD",
  ).locale("fa")
  ...
  // ایجاد هفته‌ها
  while (
    currentDate.isBefore(endOfMonthJalali) ||
    currentDate.isSame(endOfMonthJalali, "day") ||
    currentDate.isBefore(startOfMonthJalali.clone().endOf("jMonth"))
  ) {
    const days: Date[] = []
  ...
  return weeks.map(
    (week) => week.map((day) => moment(day).locale("fa").toDate()), // تبدیل به میلادی
  )
}
