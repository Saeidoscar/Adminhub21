...
function unwrapList<T>(payload: Record<string, unknown> | null | undefined, key: string) {
  const maybe = payload?.[key]
  return Array.isArray(maybe) ? (maybe as T[]) : []
}

function unwrapItem<T>(payload: Record<string, unknown> | null | undefined, key: string) {
...
export async function getAdminProfile(id: string): Promise<AdminProfile | null> {
  const payload = await apiFetch<Record<string, unknown>>(`/api/admin-profiles/${id}`)
