...
export type McpEvent =
  | { type: 'tool_started'; tool: string; input: Record<string, unknown> }
...
export async function detectIntent(text: string, opts?: McpCallOptions): Promise<McpIntent> {
  if (!config.baseUrl) {
  ...
  try {
    const res = await fetch(`${config.baseUrl}/api/mcp/intent`, {
      ...
      body: JSON.stringify({ text, language: opts?.language ?? 'en', role: opts?.role ?? 'employer' }),
    })
    if (!res.ok) throw new Error(`HTTP ${res.status}`)
...
export async function callTool(
  ...
  try {
    const res = await fetch(`${config.baseUrl}/api/mcp/tools/${toolName}`, {
      ...
      body: JSON.stringify({ input, language: opts?.language ?? 'en', role: opts?.role ?? 'employer' }),
    })
    if (!res.ok) throw new Error(`HTTP ${res.status}`)
...
export async function converse(
  ...
  try {
    const res = await fetch(`${config.baseUrl}/api/mcp/converse`, {
      ...
      body: JSON.stringify({ text, session, language: opts?.language ?? 'en', role: opts?.role ?? 'employer' }),
    })
    if (!res.ok) throw new Error(`HTTP ${res.status}`)
...
const mockTools: McpTool[] = [
  { name: 'find-admin', description: 'Find matching admins for a project', category: 'search' },
  { name: 'generate-contract', description: 'Generate a service contract template', category: 'contract' },
  { name: 'suggest-pricing', description: 'Suggest competitive pricing for services', category: 'analytics' },
  { name: 'build-brief', description: 'Build a project brief from natural language', category: 'content' },
  { name: 'recommend-team', description: 'Recommend a team composition for a budget', category: 'search' },
  { name: 'content-calendar', description: 'Generate a content calendar for social platforms', category: 'content' },
  { name: 'create-thumbnail', description: 'Generate thumbnail design suggestions', category: 'content' },
  { name: 'analyze-engagement', description: 'Analyze engagement metrics and suggest improvements', category: 'analytics' },
]

function mockIntentDetection(text: string): McpIntent {
  const lower = text.toLowerCase()
  if (lower.includes('instagram') || lower.includes('hire') || lower.includes('استخدام') || lower.includes('اینستاگرام')) {
    return { intent: 'find_admin', entities: { platform: 'instagram' }, confidence: 0.92 }
  }
  if (lower.includes('contract') || lower.includes('قرارداد')) {
    return { intent: 'generate_contract', entities: {}, confidence: 0.88 }
  }
  if (lower.includes('price') || lower.includes('cost') || lower.includes('قیمت') || lower.includes('نرخ')) {
    return { intent: 'suggest_pricing', entities: {}, confidence: 0.85 }
  }
  if (lower.includes('budget') || lower.includes('بودجه') || lower.includes('team') || lower.includes('تیم')) {
    return { intent: 'recommend_team', entities: {}, confidence: 0.8 }
  }
  return { intent: 'ask_general', entities: {}, confidence: 0.5 }
}

function mockToolCall(toolName: string, _input: Record<string, unknown>, opts?: McpCallOptions): McpResult {
  ...
  const text = key ? (isFa ? responses[key].fa : responses[key].en) : `Tool "${toolName}" is not available in mock mode.`
...
async function mockConverse(
  ...
  const aiResponse = result.text ?? (isFa
